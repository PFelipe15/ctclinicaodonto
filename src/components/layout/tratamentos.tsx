      ...
      <Carousel className="flex">
        <CarouselContent className=" ">
          {exames.map((exame: Exame, index: number) => (
            <CarouselItem
              key={index}
              className=" flex text-white items-center justify-center sm:basis-full md:basis-1/2 lg:basis-1/3 h"
            >
              <Link href={exame.link} key={index}>
                <div
                  key={index}
                  className="bg-primary rounded-lg  max-w-sm  shadow-lg p-6 transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl"
                >
                  <Image
                    src={exame.imagem}
                    alt={exame.nome}
                    width={400}
                    height={400}
                    className="rounded-lg ob mb-4"
                  />
                  <h2 className="text-xl  font-semibold mb-2">
                    {exame.nome}
                  </h2>
                  <p className=" max-w-sm">{exame.descricao}</p>
                </div>
              </Link>
            </CarouselItem>
          ))}
